...
class TopologyGeneratorTest {
    ...
    class Determinism {
        ...
        void sameSeedSameWorld() {
            for (int i = 0; i < 200; i++) {
                SoloSave first = NetTestKit.world(seed(i));
                SoloSave second = NetTestKit.world(seed(i));
        ...
        void drawCountIsAPureFunctionOfShape() {
            ...
            for (int i = 0; i < 2_000; i++) {
                long start = seed(i);
                SoloSave save = NetTestKit.world(start);
        ...
        void generateIsIdempotent() {
            SoloSave save = NetTestKit.world(seed(7));
