 ...
 * drifting apart. That only holds if {@link SoloGame} is genuinely indifferent to where its state
 * lives.
 *
...
class StoreAgnosticEngineTest {
    ...
    void opensWithoutAFile() {
        ...
        SoloGame game = SoloGame.open(store, "ghost", at(Instant.parse("2026-08-02T12:00:00Z")));
    ...
    void roundTripsThroughTheStore() {
        ...
        SoloGame first = SoloGame.open(store, "ghost", at(start));
        String characterId = first.state().characterId;
        first.persist();

        SoloGame reopened = SoloGame.open(store, null, at(start.plus(Duration.ofMinutes(5))));
    ...
    void theEngineActuallyRuns() {
        ...
        Instant start = Instant.parse("2026-08-02T12:00:00Z");
        InMemoryStore store = new InMemoryStore();
        SoloGame game = SoloGame.open(store, "ghost", at(start));
        long chainAtStart = game.chainHeight();
        game.persist();

        SoloGame later = SoloGame.open(store, null, at(start.plus(Duration.ofHours(6))));
        later.resume();
