...
### 1.5 The detection and legality material, which this document does not own
...
Concepts therefore **currently owned by nobody**, listed so the gap is visible rather than discovered later:
...
Raised as **CT-1**. The recommendation there is a document of their own rather than a merge, and the reasoning is given. Under the resolved numbering that document would be `08-detection-and-defence.md`.
...
- **CT-1: detection, logging, anti-forensics and the illegality of hacking back have no owner — still open after ED-3.** The resolved `00-curriculum-and-method.md` §1.4 stopped *claiming* these are here, which makes the gap honest, but it does not fill it. The orphans are listed in §1.5, and one of them — `cross-view-detection(7)` — is named in the contract's own §7.2 as the highest-priority audit target in the entire doc set, so it cannot simply be dropped. Another, `hack-back(7)`, is the one page in the game that tells a player specifically not to do something, and `../client/04-terminology-and-education.md` §2.8 makes it mandatory. **Three options.** (a) An eighth document, `08-detection-and-defence.md`, owning detection, logging, anti-forensics, honeytokens and hack-back. (b) Fold them into `03-operating-systems.md`, which already owns logs and processes and where cross-view detection arguably belongs anyway. (c) Widen this document. **Recommendation: (a), with `cross-view-detection(7)` going to `03` under (b) regardless**, because it is fundamentally about reading two views of a process table and its prerequisites are all `03`'s. Option (c) is worst: it would make this document carry two unrelated bodies of knowledge, and it would break the §1.4 ordering rule outright — detection material depends on the operating system in a way cryptography does not, and an eighth document sits above `03` while this one does not need to. **Decide before either document is drafted.** ⚠ Note that (a) places the material at `08`, *above* distributed systems, which is legal only because nothing in `07` will name a detection concept in `prerequisites`. Check that when the document is scoped.
...
- **CT-3: the adversarial stage is oversubscribed.** This document alone places nineteen entries there against a whole-curriculum budget of roughly twenty-five to forty (§6.2), and `07` will place most of the remainder. Either the budget is wrong for a game whose entire back half is about adversarial trust, or several entries here should move to `investigating`. The candidates for moving are `public-key-cryptography(7)`, `digital-signature(7)` and `trust-anchor(7)`, all of which a player could reasonably want the moment they own something worth defending. **Total 05 and 06 before either ships**, and note that moving any of the three drags its dependants under R1.
...
- **CT-7: the `roll-your-own(7)` self-disclosure needs an owner.** The page currently states that this project's transport layer is unreviewed, because that is true and because the honesty is the lesson. If `../architecture/07-transport-security.md` T-1 is resolved — a cryptographer reviews it, or a reviewed Noise library replaces it — the page becomes false and nothing in the current process would catch that. This is `../client/04-terminology-and-education.md` §6 **T-11** (semantic drift) with a named instance, and the `hook`/`notes` fields are the hook for it. **Whoever closes architecture T-1 must bump this entry's `revision`.**
