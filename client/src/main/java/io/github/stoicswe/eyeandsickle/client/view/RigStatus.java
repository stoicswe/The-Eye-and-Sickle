...
public record RigStatus(
    ...
    public static RigStatus of(GameSession session) {
        ...
        return new RigStatus(
                ...
                outwardCycles(budget),
                session.connected());
    }

    /**
     * Cycles committed to work that reaches OTHER machines.
     *
     * <p>⚠ This is the whole basis of the noise reading, and getting it wrong inverts the game's
     * central risk lesson. {@code docs/design/08-stealth-and-noise.md} §1 is explicit that "noise is
     * generated by <b>acting</b>", and two invariants say which acts do not count:
     *
     * <ul>
     *   <li><b>I4</b> — self-mining is structurally immune to detection and generates zero heat. It
     *       is the income floor; a floor that made you loud would not be one.
     *   <li><b>I9</b> — defending your own rig never generates heat. "Being wanted must come from
     *       aggression, not self-defense."
     * </ul>
     *
     * <p>{@code docs/design/04-mining.md} §3.1 adds the third: scanning your own rig is free and
     * silent. So a busy rig is not a loud rig — a player can run at 100% load on self-mining,
     * defences and local scans and be completely quiet, which is precisely the strategy the economy
     * is built to reward.
     *
     * <p>{@code DEPLOYED_MINER} is deliberately excluded even though it reaches across a network:
     * by <b>I6</b> a deployed miner is charged to the <em>host's</em> rig, so seeing one in your own
     * budget means somebody else's miner is running on you. That is their noise, not yours, and
     * counting it would make being a victim look like being an aggressor.
     */
    private static long outwardCycles(ComputeBudget budget) {
        return budget.allocations().stream()
                .filter(a -> !a.isRecovering())
                .filter(a -> switch (a.consumer()) {
                    case CONTROL_CHANNEL, RELAY_HOP, BOT_FRAME -> true;
                    case SELF_MINING, DEFENSIVE_ARRAY, ACTIVE_TOOL, DEPLOYED_MINER -> false;
                })
                .mapToLong(a -> a.cycles().cycles())
                .sum();
    }
    ...
    /**
     * How loud the rig is, 0–1 — <b>not</b> how busy it is.
     *
     * <p>See {@link #outwardCycles}. A rig at full load on self-mining and defences reads zero here,
     * and that is correct rather than a bug: the quiet strategy is supposed to be quiet.
     */
    public double noise() {
        long total = budget.total().cycles();
        return total == 0 ? 0 : Math.min(1.0d, outwardCycles / (double) total);
    }
