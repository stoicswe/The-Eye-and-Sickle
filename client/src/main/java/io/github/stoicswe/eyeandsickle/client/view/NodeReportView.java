    ...
    private static void paint(VBox into, NodeReport report, Instant now) {
        ...
        if (report == null || !report.any()) {
            into.getChildren().add(Ui.micro(
                    "Nothing on file. Port-scan this machine and the findings collect here."));
            return;
        }

        into.getChildren().add(micro("opened     " + stamp(report.createdAt())));
        into.getChildren().add(micro("updated    " + stamp(report.updatedAt())
                + "   (" + age(report.updatedAt(), now) + ")"));
        into.getChildren().add(micro("scans      " + report.scans()
                + (report.detections() > 0
                        ? "   ·  noticed you " + report.detections()
                                + (report.detections() == 1 ? " time" : " times")
                        : "   ·  never noticed you")));
        into.getChildren().add(micro("complete   " + report.known() + " of " + NodeReport.total()
                + " findings"));
        into.getChildren().add(new Label(" "));

        into.getChildren().add(finding(report, PortScanTarget.FIREWALL, now,
                report.firewallTier() < 0 ? null : "tier " + report.firewallTier()));
        into.getChildren().add(finding(report, PortScanTarget.OS_VERSION, now,
                report.osName().isBlank() ? null : report.osName()));
        into.getChildren().add(finding(report, PortScanTarget.CYCLE_CAPABILITY, now,
        ...
        into.getChildren().add(finding(report, PortScanTarget.CYCLE_LOAD, now,
                report.cyclesUsed() < 0
                        ? null
                        : report.cyclesUsed() + " used · " + report.cyclesFree() + " free"));
        into.getChildren().add(finding(report, PortScanTarget.DOWNLOADS, now,
                report.downloadsBytes() < 0
                        ? null
                        : String.format(Locale.ROOT, "%.1f MB", report.downloadsBytes() / 1_000_000.0d)));
        into.getChildren().add(finding(report, PortScanTarget.VAULT_HIGH, now,
                report.vaultHighCount() < 0 ? null : report.vaultHighCount() + " items"));
        into.getChildren().add(finding(report, PortScanTarget.VAULT_MEDIUM, now,
                report.vaultMediumEstimate() < 0
                        ? null
                        : report.vaultMediumLow() + "-" + report.vaultMediumHigh()
                                + " items  (estimate)"));
    }
